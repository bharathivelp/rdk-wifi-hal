...
static void *rdk_hal_server_func(void *arg)
{
    ...
    /* Ensure socket file is created with restrictive permissions */
    umask(0077);
    ...
    if (chmod(p_ipc_node->node_path, 0600) != 0) {
        wifi_hal_error_print("%s:%d chmod failed\n", __func__, __LINE__);
    }
    ...
    while ((cli_sock = accept(p_ipc_node->srv_sock, (struct sockaddr *)&cli_sockaddr, &len)) != -1) {
        /* Validate peer credentials when supported to prevent unauthorized local users */
#ifdef SO_PEERCRED
        {
            struct ucred cred;
